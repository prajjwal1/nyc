...
- iter 181-188: **categorizer FP audit + real-data driven fixes**. Bare `'social '` parties keyword was 50% FP on current data ("social media", "social tees", "social movement") — replaced with 8 specific phrases (iter 183). Bare `' park'` leading-space outdoor-strong-signal was matching `' parking'` (iter 184 — MSG concert with "free parking" wrongly kept outdoors). Bare `'discusses'` books keyword was matching "CEO discusses Q3" / "Court discusses precedent" — replaced with `'author '` trigger (iter 186). Real-data audit of remaining `[other]`-only events: `'book chat'` / `'festival'` (special) / seasonal `'summer social'` / `'sunset cruise'` keywords added (187-188). Now 99.3% of events get a meaningful category (was 99% before this pass). Categorizer is in a clean state — only 1 truly ambiguous event ("June 3 Jordin Sparks" — no inferable signal without proper-noun artist knowledge).
- iter 189-193: long-tail finishing touches. `' msg '` shorthand added to indoor-arena false-positive list so "Concert at MSG" doesn't keep outdoors tag (iter 190). Self-correction iter 191 reverted a ' party' (leading-space) change after real-data audit found 0 hyphen-party FPs vs 3 real "Party ..." TPs. Comment hygiene (iter 192-193) clarified the categorizer/quality-block interaction for reggaeton and fixed a self-referential iter-tag in iter 192's own note.
- iter 195: **cross-file drift caught** — iter 183 fixed bare `'social '` in `CATEGORY_KEYWORDS['parties']` but the SAME bare `'social'` was also in `quality.SOCIAL_KEYWORDS` (ranking-boost list). Half-fixed. Iter 195 removed the second occurrence so 'social media' / 'social tees' / 'social movement' no longer get the social-event ranking lift either. Pattern lesson: when changing a keyword, search every list using that keyword across files.
- iter 197: real-data FP — bare `'rowing'` (outdoors keyword) substring-matched `'throwing'` ("Throwing a sober Y2K party" got `[parties, outdoors]`). Changed to `' rowing'` (leading space). Genuine rowing events still match via leading-space ("Go Rowing", "Hudson River Rowing Club").
- iter 198: **fb-100 addressed** — first interactive calibration ask. User selected ALL THREE high-conviction events from `@bookclubbar` and `@litclub.nyc`. Strongest validation signal: the iter 73-109 enrichment chain is surfacing real attend-target events.
- iter 199-200: calibration-driven action. Added 9 book-event phrasings ("Books and Bourbon", "Bookish Brunch", "Page-Turners Night", "Read-Along Saturday", "Story Hour for Adults", etc.) (iter 199). Bumped books category boost 1.15 → 1.3 to match games — books events now rank competitively with the user's other top categories (iter 200).
- iter 201-203: `_IG_ACCOUNT_TOPIC_HINTS` audit completion. `exploration` joins `_VENUE_CONTEXT_CATS` so "Pop-Up Book Discussion" from `@bookclubbar` gets books from the account hint (201). Added `fiction`/`novel`/`literature` to books-hint so `@centerforfiction` gets ['books'] (202). Added dance + wellness top-level keys to the IG-account-topic-hints map — previously `@salsa_nyc` / `@yogaspace.nyc` returned no topic categories (203).
