...
export default function CommunitiesPage() {
  ...
  useEffect(() => {
    loadCommunities().then(d => setCommunities(d.communities || [])).finally(() => setLoading(false));
    queueMicrotask(() => { try { setAvailability(JSON.parse(localStorage.getItem(AVAIL_KEY) || "[]")); } catch {} });
  }, []);
  ...
  return <main className="min-h-screen bg-[#f8f3e8] px-4 py-10 sm:px-6"><div className="mx-auto max-w-7xl">
    ...
    <section className="sticky top-[85px] z-30 -mx-4 mt-10 border-y border-[#d8d0c1] bg-[#f8f3e8]/95 px-4 py-4 backdrop-blur sm:top-[53px] sm:mx-0 sm:rounded-2xl sm:border">
    ...
    <div className="my-6 flex items-center justify-between"><h2 className="font-editorial text-2xl font-bold">{loading?"Looking around the city…":`${visible.length} communities`}</h2><span className="text-xs text-[#6c766e]">Observed recurring activity · Source-linked</span></div>
